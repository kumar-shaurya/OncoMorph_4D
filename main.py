...
import numpy as np
import matplotlib
# Force headless rendering backend (disables Tkinter and prevents thread crashes)
matplotlib.use("Agg")
...
def get_slice(
    ...
    if plane == "axial":
        if index < 0 or index >= z_dim:
            raise HTTPException(status_code=400, detail="Axial slice index out of range.")
        slice_2d = volume[:, :, index]
        mask_2d = mask_volume[:, :, index] if mask_volume is not None else None

    elif plane == "coronal":
        if index < 0 or index >= y_dim:
            raise HTTPException(status_code=400, detail="Coronal slice index out of range.")
        slice_2d = volume[:, index, :]
        mask_2d = mask_volume[:, index, :] if mask_volume is not None else None

    elif plane == "sagittal":
        if index < 0 or index >= x_dim:
            raise HTTPException(status_code=400, detail="Sagittal slice index out of range.")
        slice_2d = volume[index, :, :]
        mask_2d = mask_volume[index, :, :] if mask_volume is not None else None

    else:
        raise HTTPException(status_code=400, detail="Invalid plane. Use axial, coronal, or sagittal.")

    # Rotate orientations
    slice_2d = np.rot90(slice_2d)
    if mask_volume is not None and mask_2d is not None:
        mask_2d = np.rot90(mask_2d)

    # Object-oriented figure creation (100% thread-safe, no pyplot global state)
    fig = Figure(figsize=(5, 5), dpi=120, facecolor="black")
    ...
    if show_mask and mask_volume is not None and mask_2d is not None and np.any(mask_2d):
        # 0=Transparent, 1=Necrotic(Blue), 2=Edema(Yellow), 3=Non-enhancing(Light Blue), 4=Enhancing(Red)
        class_colors = ListedColormap(["#00000000", "#0000ffff", "#ffff00ff", "#0096ffff", "#ff0000ff"])
        
        # Display the distinct colored regions
        ax.imshow(np.ma.masked_where(mask_2d <= 0, mask_2d), cmap=class_colors, alpha=0.62, vmin=0, vmax=4)
        
        # REMOVE OR COMMENT OUT THIS LINE SO IT DOESN'T DRAW A RED BORDER AROUND EVERYTHING:
